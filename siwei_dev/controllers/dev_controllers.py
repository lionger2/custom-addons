...
class DevControllers(http.Controller):
    ...
    @http.route('/dev/module', type='http', auth="user")
    ...
    def _make_model_info(self, env, module):
        ...
        for model in models:
            ...
            '  access:' + str(model.access_ids) + '  rule:'   + str(model.rule_ids) + '  transient:' + str(model.transient) + '  modules:' + model.modules \
             + '\n' 
             
        str_content += '\n\n' + '================== Extended Models ======================' + '\n'
        extend_models = Model.search(['|', ('modules', 'like', ', '+module.name+','), ('modules', '=like', '%, '+module.name+'')])
        for extend_model in extend_models:
            str_content += '\n###' + extend_model.model + ' name:' + extend_model.name + ' inherit:' + str(extend_model.inherited_model_ids) + \
        ...
        for model in models:
            ...
            for action in actions:
               str_content += '\n###' + action.xml_id + '  name:' + action.name + ' view_mode:' + action.view_mode + ' domain:' + str(action.domain) + \
        ...
        for model in models:    
            ...
            for view in model.view_ids:
                str_content += '\n' + view.xml_id + '  type:' + view.type + '  priority:' + str(view.priority) + '  arch_fs:' + view.arch_fs + '  inherit_id:' + str(view.inherit_id.xml_id if view.inherit_id else '') + '\n'
        ...
        for qweb in qwebs:    
            str_content += '\n' + str(qweb.key) + '  type:' + qweb.type + '  priority:' + str(qweb.priority) + '  arch_fs:' + str(qweb.arch_fs) + '  inherit_id:' + str(qweb.inherit_id.xml_id if qweb.inherit_id else '') + '\n'
        ...
        for model in models:
            ...
            for field in model.field_id:
                ...
                str_content += '\n'+ field.name + ' Label:' + str(field.field_description) + '  ttype:' + field.ttype + "  " + relation + '  required:' + str(field.required) + '  compute:' + str(field.compute) + '  depends:' + str(field.depends) + '  modules:' + field.modules + '  help:' + str(field.help)
